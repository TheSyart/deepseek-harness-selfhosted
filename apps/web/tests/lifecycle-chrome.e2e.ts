...
const ADD_MENU_EXPECTED = join(SNAPSHOT_DIR, 'add-menu.expected.md')
...
describe('web e2e: lifecycle & chrome (workspace flow / reload / dark mode)', () => {
  ...
  it.skipIf(MODE === 'record')('opens the add-content menu, then the shared slash menu with only Command candidates', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-command-menu-launcher'))
    const launcher = page.getByRole('button', { name: 'Add content' })
    await launcher.click()
    const addMenu = page.getByRole('menu')
    await addMenu.waitFor({ timeout: 10_000 })
    const addSnapshot = await captureStableAria(page, '[role="menu"]', scaffold.workspaceCwd)
    await compareOrRefreshGolden(ADD_MENU_EXPECTED, addSnapshot, MODE)
    expect(await addMenu.getByRole('menuitem').allTextContents()).toEqual(['Commands', 'Add photo'])
    await addMenu.getByRole('menuitem', { name: 'Commands' }).click()
  ...
  it.skipIf(MODE === 'record')('keeps the fixture inventory closed', async () => {
    ...
    await assertFixtureInventory(SNAPSHOT_DIR, [
      'session.jsonl', 'replay.override.json', 'add-menu.expected.md', 'command-menu.expected.md', 'command-menu-fuzzy.expected.md', 'hero.expected.md', 'plan-active.expected.md', 'reloaded.expected.md',
    ])
  })
})
