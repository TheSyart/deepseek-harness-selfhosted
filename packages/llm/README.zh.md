...
| [`token-usage-report/`](token-usage-report/README.zh.md) | 全局提供方上报用量 Remote | `ctx.tokenUsageReport` |
...
适配器在 seam 上注册提供方路由；重试、token 测量与 Host 侧用量报告仍是独立消费方。子 README 负责路由、元数据、回放和提供方协议细节；[LLM 架构决策](../../.agents/notes/implemented/architecture/2026-06-13-twin-llm-adapters.zh.md)说明设计原理。
