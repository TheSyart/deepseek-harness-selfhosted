...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-shuangwen': { kind: 'indirect', reason: 'Quick actions stage or submit ordinary user messages; the selected preset owns system-prompt composition.' },
  ...
  'packages/client/ui-settings-token-usage': { kind: 'none', reason: 'Browser-side settings report over already-logged usage; registers nothing model-facing.' },
  ...
  'packages/llm/token-usage-report': { kind: 'none', reason: 'The Host report reads already-logged provider usage and registers nothing model-facing.' },
