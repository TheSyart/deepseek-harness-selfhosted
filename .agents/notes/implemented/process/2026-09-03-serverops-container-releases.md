...
The release manager can bind image bytes to source and preserve data independently. The image build fails on the existing missing ui-shuangwen workspace dependency instead of substituting a host binary. Frozen installation, packaged startup and actual Linux readiness remain required checks; no container runtime verification is claimed. Existing SDK packaging and CLI host validation remain unchanged.
