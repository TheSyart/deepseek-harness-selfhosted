...
RUN pnpm --filter @deepseek-ai/dsh deploy --legacy --prod /out \
    && node /out/lib/bin.js --version \
    && node /out/lib/bin.js --profile web --help \
    && node -e "const {createRequire}=require('node:module'); const cli=createRequire('/out/package.json'); const web=createRequire(cli.resolve('@deepseek-ai/dsh-web-app')); require('node:fs').accessSync(web.resolve('@deepseek-ai/dsh-web-frontend/dist/index.html'))"
...
CMD ["node", "/app/runtime/lib/bin.js", "--profile", "web", "--patch", "/app/container.patch.yml", "--no-open"]
